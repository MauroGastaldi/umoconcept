...
const ItemDetail = ({ id, nombre, precio, ancho, alto, img, stock, imagenesSecundarias = [], colores = [] }) => {
  const [agregarCantidad, setAgregarCantidad] = useState(0);
  const [colorSeleccionado, setColorSeleccionado] = useState(colores[0] || ""); // Color por defecto
  const { agregarAlCarrito } = useContext(CarritoContext);

  const manejadorCantidad = (cantidad) => {
    setAgregarCantidad(cantidad);
    const item = { id, nombre, precio, img, ancho, alto, color: colorSeleccionado };
